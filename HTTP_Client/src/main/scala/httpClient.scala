...
object httpClient {
  ...
  def sendRequest(start:String, delta:String): Future[String] = {
    // sends HTTP request to API Gateway and waits for HttpResponse
    val responseFuture: Future[HttpResponse] = Http().singleRequest(HttpRequest(uri = s"https://qnvqyz4nka.execute-api.us-east-2.amazonaws.com/test/indexes?start=$start&delta=$delta"))
  ...
  def main(args: Array[String]): Unit = {
    ...
    val data = sendRequest(params.getString("start"), params.getString("delta"))
    data.foreach(x => {
