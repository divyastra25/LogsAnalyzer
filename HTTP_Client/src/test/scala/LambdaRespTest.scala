...
class LambdaRespTest extends AnyFlatSpec with Matchers {
  ...
  it should "Find 25 Logs within the Interval" in {
    val myRespData = parse(Await.result(httpClient.sendRequest(params.getString("date"), params.getString("start"), params.getString("delta")), 5.seconds))
  ...
  it should "Find 0 Logs and return \"Interval not present or No logs in the interval\"" in {
    val myRespData = parse(Await.result(httpClient.sendRequest(params.getString("date"), params.getString("invalidIntervalStart"), params.getString("invalidIntervalDelta")), 5.seconds))
  ...
  it should "Find 0 Logs and return \"Incorrect Date/Format or No Logs For The Date: 2012-07-01\"" in {
    val myRespData = parse(Await.result(httpClient.sendRequest(params.getString("invalidDate"), params.getString("start"), params.getString("delta")), 5.seconds))
  ...
  it should "Find 0 Logs and return \"Time input format incorrect. Enter in HH:MM:SS format.\"" in {
    val myRespData = parse(Await.result(httpClient.sendRequest(params.getString("date"), params.getString("invalidStartFormat"), params.getString("invalidDeltaFormat")), 5.seconds))
  ...
  it should "Find 0 Logs and return \"Incorrect Date/Format or No Logs For The Date: 02-28-2017\"" in {
    val myRespData = parse(Await.result(httpClient.sendRequest(params.getString("invalidDateFormat"), params.getString("start"), params.getString("delta")), 5.seconds))
